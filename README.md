...
download the latest tak server and place it in `takgov_assets` example `takgov_assets/takserver-docker-5.5-RELEASE-45.zip`
for more [info](takgov_assets/README.md)
...
```
docker compose -p tak exec takserver_api /bin/bash -c 'base64 /opt/tak/certs/files/clientpkgs/replaceme.zip' | base64 -d > replaceme.zip
